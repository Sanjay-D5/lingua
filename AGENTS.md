...
| **Image size (local assets)**  | RN auto-sizes a local `require()`'d image to its native pixel dimensions unless `style.width`/`style.height` is set, and that always overrides a NativeWind className | Inline `style` for `width`/`height` only |
...
```tsx
// ✅ CORRECT - width/height via inline style, everything else can stay className
<Image
...
// ❌ INCORRECT - className loses to RN's auto intrinsic-size behavior
